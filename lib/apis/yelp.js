...
export async function getNeighborhoodContext(lat, lng, city, state) {
  ...
  const categories = [
    ...
    { key: 'fitness', label: 'Gyms & Fitness' },
  ...
  for (const { key, label } of categories) {
    try {
      console.log(`🔍 [YELP] Searching for ${label} (category: ${key})...`)
      ...
      console.log(`✅ [YELP] Found ${data.total} ${label} within 1 mile`)
      ...
      console.error(`❌ [YELP] Error fetching ${key}:`, error.message)
      categoryResults.push({
        category: key,
        label,
        count: 0,
  ...
  neighborhoodData.summary = {
    ...
    hasFitness: (neighborhoodData.statistics.fitness || 0) > 0,
    hasSchools: (neighborhoodData.statistics.schools || 0) > 0,
